...
5번 CCTV : 1가지
~~~
따라서, CCTV번호를 인덱스로, 값은 회전 경우의 수로 정의하는 배열을 만든다. (0번 CCTV는 없으므로 0번 인덱스는 0으로 초기화)
~~~
...
cctv를 담는 배열을 선언하고, 재귀함수를 통해 cctv배열에서 하나의 cctv를 선택한 후, 해당 cctv의 type을 통해 방향을 결정한다.  
방향이 결정되면, 해당 방향에 대해 cctv탐색을 진행하도록 한다.
